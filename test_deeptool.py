...
ct = mb._make_gradient_colortable(mb.DEFAULT_GRADIENT_STOPS)
...
try:
    original_light = action_state["light_angle"]
    overlay._do_action("light_angle", "plus", action_state)
    check("_do_action light_angle plus: changes", action_state["light_angle"] != original_light)
except Exception as e:
    check(f"_do_action light_angle: no crash ({type(e).__name__})", False, str(e))
...
try:
    def _capture(text):
        _copied_texts.append(text)
        return True
    mb._set_clipboard = _capture
    for _key, _pt_key in (("coord-copy-m", "orbit_point_m"),
                          ("coord-copy-j", "orbit_point_j"),
                          ("coord-copy-c", "julia_c")):
        _th = overlay5._copy_coord(_key, coord_state)
        if _th is not None:
            _th.join(timeout=2.0)
    check("_copy_coord: M/J/C each invoke clipboard",
          len(_copied_texts) == 3, f"got {len(_copied_texts)}")
    check("_copy_coord: copies formatted coordinates",
          all(isinstance(t, str) and "i" in t for t in _copied_texts))
    _th = overlay5._copy_coord("coord-copy-nonexistent", coord_state)
    check("_copy_coord: invalid key copies nothing", _th is None)
finally:
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    mb.save_settings(settings_file, {"max-iterations": "1024"})
    val = mb.get_persistent_setting({"max-iterations": "1024"}, "max-iterations", cast=int, default=256)
    check("get_persistent_setting: int", val == 1024)
    val_str = mb.get_persistent_setting({"max-iterations": "1024"}, "max-iterations", cast=str, default="256")
